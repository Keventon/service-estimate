...
import { deleteService, getServiceById, upsertService } from "@/service/storage";
...
export default function EditService() {
  ...
  const [selectedStatus, setSelectedStatus] = useState<ServiceStatus>("draft");
  ...
  const [servicesIncluded, setServicesIncluded] = useState<ServiceItem[]>([]);
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      async function load() {
        ...
        setDiscountPercent(String(response.discountPercent ?? 0));
        setEstimateTitle(response.title);
        setClientName(response.client);
        setServicesIncluded(response.services ?? []);
      }
      load();
      return () => {
        isActive = false;
  ...
  function populateServiceFields(service: ServiceItem) {
  ...
  function handleDeleteServiceItem() {
    ...
    setServicesIncluded((prev) => prev.filter((item) => item.id !== editingServiceId));
    handleCloseBottomSheet();
  }

  const hasDiscount =
    (Number(discountPercent) || 0) > 0 && discountValue > 0;
  ...
  async function handleConfirmDelete() {
    if (!currentService || isDeleting) return;
    setIsDeleting(true);
                  ...
                  <View style={styles.discountBadge}>
                    <TextInput
                      ...
                      onChangeText={(value) => {
                        ...
                        setDiscountPercent(onlyNumbers);
                      }}
                      placeholder="0"
        ...
        <BottomSheetView style={styles.sheetContainer}>
          ...
          <View style={styles.sheetFooter}>
            <TouchableOpacity
              ...
              onPress={handleDeleteServiceItem}
            >
              <Trash2
                size={18}
                strokeWidth={2}
